...
//| echo: false
//| output: false
// A function, not a static constant: the number of branches depends
...
clusterPaint = (clusterIds) => {
  // A real case, not a hypothetical one: sparse real-world data at the
  // default ε/min points can come back as ALL noise, zero real
  // clusters -- a MapLibre "match" expression needs at least one
  // (value, output) pair before its fallback, so an empty clusterIds
  // would build an invalid expression (MapLibre's own error: "Expected
  // at least 4 arguments, but found only 2"). A flat color sidesteps
  // that entirely when there's nothing to differentiate anyway.
...
statCard = {
  ...
  if (r) {
    ...
    if (r.excludedCount > 0) {
      row("Excluded from clustering", `${r.excludedCount} (too far from the calculation UTM zone to project)`);
    }
