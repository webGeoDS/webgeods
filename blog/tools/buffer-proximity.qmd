...
if dissolveBuffers:
    # Dissolved WITHIN each ring level, across every feature -- "the
    # total area within ring N of ANY feature", not of each one
    # separately. A simplification worth naming: two features close
    # enough for their rings to overlap EACH OTHER (not just their own
    # rings) can leave a sliver double-counted between ring colors --
    # a full pairwise overlay would resolve it exactly, at a cost not
    # worth paying for this tool's scope.
    dissolved_rows = []
    for i, distance in enumerate(ring_distances):
        level = ring_gdf[ring_gdf["ring"] == i + 1]
        dissolved_geometry = level.geometry.union_all()
    ...
    ring_gdf = gpd.GeoDataFrame(dissolved_rows, crs=gdf_proj.crs)
