...
description: "Upload points, lines, or polygons and generate a buffer at any distance — see what's within reach of each feature, optionally dissolved into one shape, colored on the map, downloadable. Runs entirely in your browser, powered by Python."
page-layout: full
---
...
"what falls within 50m of this road."
...
#| inject: [bufferDistance, dissolveBuffers]
#| package: geopandas
gdf_calc = gdf.copy()
...
buffered["geometry"] = gdf_proj.geometry.buffer(float(bufferDistance))

if dissolveBuffers:
    dissolved = buffered.geometry.union_all()
    buffered = gpd.GeoDataFrame({"name": ["dissolved buffer"]}, geometry=[dissolved], crs=gdf_proj.crs)

buffered_wgs84 = buffered.to_crs("EPSG:4326")

{
    "features": buffered_wgs84.__geo_interface__,
    "originalCrsFeatures": export_crs(buffered_wgs84).__geo_interface__,
    "summary": {
        "calculationCrs": calc_crs_label,
        "bufferDistance": float(bufferDistance),
        "dissolved": bool(dissolveBuffers),
        "bufferCount": int(len(buffered))
    }
}
```
...
bufferControlsRow = {
  ...
  computeButton.onclick = async () => {
    ...
    try {
      ...
      await sharedMap.setGeoJSON("buffer-result-py", result.features, { paint: BUFFER_PAINT });
      await sharedMap.fitToData(result.features);
      mutable exportFeatures = result.originalCrsFeatures;
      mutable resultSummary = { ...result.summary, bufferDistance, dissolveBuffers };
  ...
  row.append(slider, dissolveLabel, computeButton);
  return row;
}
```
...
legendEl = {
  ...
  swatch.style.background = "rgba(66,88,60,0.6)";
  const label = document.createElement("span");
  label.className = "webgeods-legend-label";
  label.textContent = `Buffer (${r.bufferDistance} m)`;
