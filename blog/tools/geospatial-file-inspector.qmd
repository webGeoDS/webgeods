...
// other tool uses. Downloads the SAME enriched GeoJSON shown on the
// map (original columns + valid/empty/duplicate), not a re-export of
// the untouched upload: this tool never modifies geometry, so there's
// nothing to "repair" here, but the three flags it computed are
// useful to keep.
downloadButton = {
  const button = document.createElement("button");
  button.className = "webgeods-panel-btn";
  ...
  button.onclick = () => {
    const data = sharedMap.getGeoJSON("inspect-py");
    if (!data) return;
