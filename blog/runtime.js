...
(() => {
  ...
  const DEFAULT_CONFIG = {
    ...
    python: {
      // Self-hosted on GitHub Pages (webGeoDS/webgeods-assets) instead
      ...
      //
      // Migrated 2026-09-07 from the personal `DSwing` account to the
      // project's own `webGeoDS` account (same account the main site
      // repo already lives on) — a fresh repo + push, not a GitHub
      // "transfer", since transfers don't redirect GitHub Pages (only
      // git clone/fetch/push), which would have broken every page of
      // the live site instantly. New content verified byte-identical
      // (size + magic bytes on a sample, matching blob count) before
      // this URL was changed.
      indexURL:
        "https://webgeods.github.io/webgeods-assets/pyodide/v0.29.4/"
    ...
    r: {
      ...
      //
      // Migrated 2026-09-07 to the `webGeoDS` account — see the same
      // note on the `python.indexURL` above for why (fresh repo +
      // push, verified byte-identical, not a GitHub transfer).
      //
      ...
      url:
        "https://webgeods.github.io/webgeods-assets/webr/v0.4.3/webr.mjs",
      baseUrl:
        "https://webgeods.github.io/webgeods-assets/webr/v0.4.3/",
      repoUrl:
        "https://webgeods.github.io/webgeods-assets/webr/repo/"
