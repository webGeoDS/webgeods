...
-- The project's `date-format: long` (_quarto.yml) rewrites meta.date
-- to a display string ("September 7, 2026") before this filter runs
-- -- confirmed by inspecting the actual rendered JSON-LD, not the raw
-- frontmatter. schema.org's datePublished needs ISO 8601
-- (YYYY-MM-DD): Google Rich Results parses that format reliably and
-- a long-form date unreliably, per Google's own structured-data docs.
-- Reparsed back here rather than passed through, with an ISO
-- passthrough guard so this keeps working if date-format ever
-- changes (or a page sets date-format: iso locally).
local function to_iso_date(s)
  if s:match("^%d%d%d%d%-%d%d%-%d%d$") then
    return s
  end
  local month_name, day, year = s:match("^(%a+) (%d+), (%d+)$")
...
function Meta(meta)
  ...
  -- Individual tool pages only -- tools/index.html is the listing
  -- page, not a tool itself, same "collection page gets no per-item
  -- schema" reasoning already applied to posts/ vs. articles.html.
  ...
  -- datePublished when the page sets a date -- most do), WebApplication
  -- for individual tool pages (free, browser-only, no install --
  -- isAccessibleForFree/browserRequirements say exactly that, no
  -- rating/price fields since none of that data actually exists),
  -- WebSite only for the actual homepage (a Person/Organization block
  -- would need real entity data this project doesn't have yet, so
  -- left out rather than filled with placeholders).
  local jsonld
  if is_post then
    local date_line = ""
    if meta.date then
      local date_str = to_iso_date(pandoc.utils.stringify(meta.date))
      if date_str ~= "" then
