...
(() => {
  ...
  class WebGeoDSMap {
    ...
    // (via getGeoJSON() above) into a Grid.js table (shared/table.js):
    ...
    // shared table — see topology-fix.qmd). For a page that wants
    // specific columns or custom labels, call WebGeoDS.Table.render()
    // directly instead — this generic dump deliberately doesn't
    // attempt that, only row-level coloring (below).
    //
    // options.rowClassName(rowValues) — optional: called once per row
    // with a plain {columnKey: stringValue} object (the SAME already-
    // stringified values being displayed, keyed by column) — return a
    // CSS class name to color that row, or a falsy value for none. The
    // class itself isn't defined here; the caller's stylesheet must
    // provide it (e.g. shared/styles.css's .webgeods-table-row-fixed/
    // -invalid). Implemented via Grid.js's per-column `attributes`
    // callback (verified empirically: `row.cells[i].data` is the
    // reliable way to read a row's OTHER column values from within
    // it — reconstructing "this row" from Grid.js's own object, not
    // from index-matching against the original features array, stays
    // correct even if Grid.js internally sorts/filters).
    //
    // Requires shared/table.js on the page — always true for blog/
    // (loaded on every page via _quarto.yml's include-after-body) but
    // NOT for lessons/ (no table.js/gridjs entry there) even though
    // both sync the same shared/map.js — checked explicitly so a
    // lessons/ page calling this fails with a clear message instead of
    ...
    table(
      ...
      // Grid.js doesn't reliably display a raw boolean cell value
      // (verified empirically: renders blank, not "true"/"false") —
      // every other WebGeoDS.Table.render() call in this project
      // already stringifies explicitly for the same reason, this is
      // the generic-dump equivalent.
      const stringify =
        (value) => {
      ...
      const rowAttributes =
        (_cell, row) => {
          ...
          if (!row) {
            ...
            // The `th` (header) call — cell/row are empty, per
            // Grid.js's own docs; nothing to color there.
            return {};
          ...
          const rowValues =
            Object.fromEntries(
              keys.map((key, i) =>
                [key, row.cells[i]?.data]
              )
            );

          const className =
            rowClassName(rowValues);
    ...
    // (not the Grid.js instance) on every render, so a plain (non
    // `output: false`) {ojs} cell assigning `tableCell(...)` displays
    // it automatically via Observable's own "a cell whose value is a
    // DOM Node gets shown" convention — same SAME element reference
    // every time, just mutated in place by Grid.js's own
    // updateConfig()/forceRender(), so it never needs to "move".
    // Passing an explicit string still works exactly as before
    // (looked up via getElementById inside table()/Table.render()).
    //
    ...
    //
    // Unlike table(), which COMBINES every given source's features
    // into one table, this shows exactly ONE source at a time:
    // whichever tracked source had its data content change MOST
    // RECENTLY. That's the shape a page with several independent
    // per-language sources feeding one map actually wants — a table
    // that always mirrors "whichever one just ran", not a merged view
    // of all of them (use table() directly for that instead).
    //
