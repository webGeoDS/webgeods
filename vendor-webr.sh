...
VFS_FILES=(
  ...
  # Not needed by any package we've vendored so far, but webR's
  # install.packages() fetches this unconditionally when installing
  # ANY package whose DESCRIPTION Priority is "recommended" (observed
  # installing codetools, itself unrelated to tcltk — confirmed via a
  # real 404 in the browser, not assumed) — apparently part of
  # ensuring the base+recommended library tree metadata is complete,
  # regardless of which specific recommended package triggered it.
  "vfs/usr/lib/R/library/tcltk.js.metadata"
  "vfs/usr/lib/R/library/tcltk.data.gz"
  # Same discovery as tcltk above, one round trip later: after tcltk,
  # install.packages() on a "recommended" package next fetched this
  # one too. Checked the remaining base-priority package names
  # (base/compiler/datasets/grDevices/graphics/grid/methods/splines/
  # stats/stats4/tools/utils) directly against webr.r-wasm.org — all
  # 403 (baked into the R.wasm core image itself, not separate
  # mountable images) — only translations, parallel and tcltk exist
  # as standalone vfs images, so this is very likely the complete set.
  "vfs/usr/lib/R/library/translations.js.metadata"
