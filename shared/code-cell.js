...
(() => {
  ...
  // languages' cell code at once (setCode() on each, in parallel) and
  // scrolls the Python cell into view. Promoted here after six
  // articles (geometry-validity, geospatial-file-inspection,
  // raster-band-math, raster-file-inspection, raster-ndvi,
  // topology-errors) turned out to define the exact same function,
  // differing only in which two cell ids they passed — a genuine
  // duplicate, not just a similar shape, confirmed by diffing all six
  // before promoting this. Takes ids as an options object rather than
  // positional pyId/rId strings so a caller can't silently transpose
  // them the way four bare string arguments in a row would invite.
  //
  // Standalone tool pages (single-language) each build their own
  // "Load example" button with tool-specific logic (busy state,
  // status messages, calling a generateExample()/autoInspect() pair)
  // — genuinely different per tool, not this same duplicate, so left
  // as they are rather than forced through this helper.
  // ============================================================

  function loadExampleButton(label, { pyId, rId, pyCode, rCode }) {
    ...
    button.onclick = async () => {
      ...
      await Promise.all([
        window.WebGeoDS.CodeCell.find(pyId)?.setCode(pyCode),
        window.WebGeoDS.CodeCell.find(rId)?.setCode(rCode)
      ]);

      document.getElementById(pyId)?.scrollIntoView({
        behavior: "smooth",
        block: "center"
      });
